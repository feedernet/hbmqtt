...
def get_open_ports(num_ports: int = 1) -> tuple:
    import socket
    sockets = []
    ports = []
...
class BrokerTest(unittest.TestCase):
    def setUp(self):
        ...
        self._test_port = get_open_ports()[0]
        self._test_config = {
            'listeners': {
                'default': {
                    'type': 'tcp',
                    'bind': f'127.0.0.1:{self._test_port}',
                    'max_connections': 10
                },
            },
            'sys_interval': 0,
    ...
    def test_client_connect(self, MockPluginManager):
        async def test_coro():
            try:
                ...
                self.assertTrue(broker.transitions.is_started())
                client = MQTTClient()
                ret = await client.connect(f'mqtt://127.0.0.1:{self._test_port}/')
        ...
        async def test_coro():
            try:
                ...
                conn_reader, conn_writer = \
                    await asyncio.open_connection('127.0.0.1', self._test_port, loop=self.loop)
    ...
    def test_client_connect_clean_session_false(self, MockPluginManager):
        async def test_coro():
            try:
                ...
                return_code = None
                try:
                    await client.connect(f'mqtt://127.0.0.1:{self._test_port}/', cleansession=False)
